...
we are going to use map_dfr, which should be interpreted as  "map dataframe by rows", which calculates dataframe for each element of the input and then returns a single dataframe that comes from stacking all of the dataframes for each element by row (like `rbind()` or `bind_rows()`)
...
This output would be helfpul except we've now lost track of which sequence corresponds to which ouput.We can use an ".id" argument (which is also available in `bind_rows`) to
keep track of the rows. The ".id" argument will be the name of a new column in the dataframe, which is populated by the name of the element of the list that generated it. This is why we are using a named list rather than an unnamed list
